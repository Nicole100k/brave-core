...
(function() {
  ...
  function uuid_v4() {
      ...
      // So '8' digit is clamped to 3 (inclusive) and all others clamped to 15
      // (inclusive).
      ...
      return ([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g, (X) => {
          return (X ^ (crypto.getRandomValues(new Uint8Array(1))[0] &
              (15 >> (X >> 2)))).toString(16);
      });
  }
  ...
  function tagNode(node) {
      if (node) {
          if (!node.tagUUID) {
              ...
              node.addEventListener('webkitpresentationmodechanged',
...
>>>>>>> b729b3fdd1 (Address code review: base::StrCat,JS script and CSP allow list)
  return kScript;
}
