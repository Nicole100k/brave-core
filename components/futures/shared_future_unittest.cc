...
#include "brave/components/futures/shared_future.h"
...
TEST_F(SharedFutureTest, MultipleListeners) {
  ...
  future.AndThen(base::BindLambdaForTesting(push_value));
  future.AndThen(base::BindLambdaForTesting(push_value));
  future.AndThen(base::BindLambdaForTesting(push_value));
...
TEST_F(SharedFutureTest, Copyable) {
  ...
  future2.AndThen(base::BindLambdaForTesting(set_value));
